...
(if (version<= "26.0.50" emacs-version )(progn
    ;; hack ... for some reason line number dissapear when you open a file, so we force this shit
    (add-hook 'window-setup-hook 'display-line-numbers-mode)   ;; faster but only available from emacs v26
    (add-hook 'menu-bar-update-hook 'display-line-numbers-mode)   ;; faster but only available from emacs v26
  )
  (progn
    ;; force upgrade to emacs 26
    (mkdir (file-truename "~/.emacs.d/emacs26") 1)
    (f-write-text "cd ~/.emacs.d/emacs26
                   ...
                   echo \"press [ENTER] to continue\"
                   read henk
                   " 'utf-8 (concat (getenv "HOME") "/.emacs.d/emacs26/install.sh"))
    (shell-command-to-string (concat "chmod +x " (concat (getenv "HOME") "/.emacs.d/emacs26/install.sh")))
))
...
(define-derived-mode todo-list-mode fundamental-mode "todo-list"
  (font-lock-add-keywords nil '(
	("\\([-]\\)" 1 font-lock-keyword-face prepend)
